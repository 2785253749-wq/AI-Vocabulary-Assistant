...
export default function AIPage() {
  ...
  const tabs: { key: TabType; label: string; icon: string }[] = [
    { key: 'example', label: '生成例句', icon: '📖' },
    { key: 'memory', label: '生成记忆法', icon: '🧠' },
    { key: 'analyze', label: '学习分析', icon: '📊' },
  ];

  const handleSubmit = async () => {
    if (tab !== 'analyze' && !word.trim()) {
      setError('请输入单词');
      return;
    }
    setError('');
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="page-container">
        <h1 className="text-3xl font-bold text-gray-800 mb-2">🤖 AI助手</h1>
        ...
        <div className="card">
          ...
          {tab !== 'analyze' && (
            <div className="flex gap-3 mb-4">
              ...
              <input
                ...
                onKeyDown={(e) => e.key === 'Enter' && handleSubmit()}
              />
              <button onClick={handleSubmit} disabled={loading} className="btn-primary whitespace-nowrap">
                {loading ? '生成中...' : '生成'}
              </button>
            </div>
          )}
          ...
          {result && (
            <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
              <p className="text-sm text-gray-800 whitespace-pre-line leading-relaxed">{result}</p>
            </div>
          )}
          ...
          {!result && !error && !loading && (
            <div className="text-center py-8 text-gray-400">
              <p className="text-3xl mb-2">
                {tab === 'example' ? '📖' : tab === 'memory' ? '🧠' : '📊'}
              </p>
              <p>
                {tab === 'analyze' ? '点击按钮开始分析' : '输入单词并点击生成'}
              </p>
            </div>
          )}
        ...
        <p className="text-xs text-gray-400 text-center mt-4">
          由 DeepSeek API 驱动 | 结果由AI生成，仅供参考
        </p>
      </main>
    </div>
  );
